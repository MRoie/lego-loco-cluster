...
Taken from containers/qemu-softgpu so both emulator flavors hand out the same
addresses. The lease is derived from the guest MAC — 192.168.10.(10 + last
byte) — so one golden disk image can be cloned to N instances and each still
gets a distinct, stable address with nothing configured inside Windows.
"""
import os
import socket
import struct
