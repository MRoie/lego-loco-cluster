...
if [ -f /usr/local/bin/setup-lan-network.sh ] && [ "$NETWORK_MODE" != "bridge" ]; then
  ...
  # Set legacy QEMU_NET_ARGS for bridge mode
...
# Build QEMU command with network args from setup-lan-network.sh or legacy bridge
QEMU_EXTRA_ARGS=""

# Add QMP socket for computer-use agent input injection
QMP_SOCKET="/tmp/qmp-${INSTANCE_ID:-0}.sock"
QEMU_EXTRA_ARGS="$QEMU_EXTRA_ARGS -qmp unix:$QMP_SOCKET,server,nowait"
log_info "QMP socket: $QMP_SOCKET"

# Add -loadvm support for fast startup if LOADVM_TAG is set
if [ -n "${LOADVM_TAG:-}" ]; then
  QEMU_EXTRA_ARGS="$QEMU_EXTRA_ARGS -loadvm $LOADVM_TAG"
  log_info "Fast startup: loading VM snapshot '$LOADVM_TAG'"
fi
