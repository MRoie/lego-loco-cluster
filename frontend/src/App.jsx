...
export default function App() {
  ...
  // Use progressive loading hook for better performance
  const {
    instances,
  ...
  useEffect(() => {
    ...
    // Set up periodic status updates after initial load
    const interval = setInterval(() => {
      // Only refresh status and provisioned instances periodically
      // Don't reload all data to avoid disrupting user experience
      if (isCriticalDataLoaded) {
        refreshData('status');
        refreshData('provisionedInstances');
      }
    }, 5000);
  ...
  }, [loadData, refreshData, isCriticalDataLoaded]);
  ...
  return (
    <>
      {/* App Loading Overlay - shown during initial load */}
      <AppLoadingOverlay 
