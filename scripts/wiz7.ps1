...
Write-Host "sendkey ret (Finish)"
& $Kube exec $Pod -n loco -- python3 /usr/local/bin/qmp-control.py sendkey ret 2>&1
Start-Sleep -Seconds 5
& $Kube exec $Pod -n loco -- python3 /usr/local/bin/qmp-control.py screendump /tmp/wiz7.ppm 2>&1
& $Kube cp -n loco "${Pod}:tmp/wiz7.ppm" "scripts/out/wiz7.ppm" 2>&1
Write-Host "=== DONE ==="
